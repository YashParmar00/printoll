...
const testimonials = [
  {
    names: "Couple name",
    city: "City, State",
    quote: "Replace this with a real review in the couple's own words. Keep their photo of the set, and never invent a rating.",
    initials: "01",
...
export default function Testimonials() {
  ...
  return (
    <section className="overflow-hidden bg-night py-14 sm:py-16 md:py-20" aria-labelledby="testimonial-heading">
      <div className="container-page">
        <div className="grid items-end gap-5 sm:grid-cols-[1fr_auto]">
          <div className="max-w-2xl">
            ...
            <h2 id="testimonial-heading" className="mt-2 text-2xl text-white sm:text-3xl md:text-4xl">
              Small matching moments. Big feelings.
            </h2>
            <p className="mt-3 text-sm leading-relaxed text-night-ink sm:text-base">
              A rotating spotlight for genuine couple stories, photos and feedback as they arrive.
            </p>
          </div>
        ...
        <div className="mt-7 grid grid-cols-2 gap-3 sm:mt-9 sm:gap-4 md:grid-cols-3 md:gap-5 lg:grid-cols-4 lg:gap-6">
          {visibleTestimonials.map((testimonial, slot) => (
            <figure
              key={`${active}-${testimonial.initials}`}
              className={`relative overflow-hidden rounded-2xl border p-4 transition duration-500 sm:p-5 lg:rounded-3xl lg:p-6 ${slot === 0 ? "border-coral/45 bg-night-card shadow-[0_18px_50px_rgba(210,96,63,0.12)] lg:-translate-y-2" : "border-night-line bg-night-card/65"} ${slot === 2 ? "hidden md:block" : ""} ${slot === 3 ? "hidden lg:block" : ""}`}
            >
              {/* <div aria-hidden className={`absolute inset-x-0 top-0 h-1 bg-gradient-to-r ${testimonial.accent}`} /> */}
              ...
              <div className="mt-4 flex items-center justify-between gap-2 border-t border-white/10 pt-3 sm:mt-5 sm:gap-3 sm:pt-4">
                ...
                <span className="flex shrink-0 gap-0.5 text-coral" aria-label="5 out of 5 stars">
                  {Array.from({ length: 5 }).map((_, star) => <StarIcon key={star} className="h-3 w-3" />)}
                </span>
              </div>
            </figure>
          ))}
        ...
        <div className="mt-6 flex items-center justify-center gap-2 sm:mt-8">
          {testimonials.map((testimonial, index) => (
            <button key={testimonial.initials} type="button" onClick={() => goTo(index)} aria-label={`Show testimonial ${index + 1}`} aria-current={active === index ? "true" : undefined} className={`h-1.5 rounded-full transition-all ${active === index ? "w-8 bg-coral" : "w-2 bg-white/25 hover:bg-white/55"}`} />
          ))}
          <span className="ml-2 text-xs text-night-ink">{String(active + 1).padStart(2, "0")} <ArrowRightIcon className="mx-1 inline h-3 w-3" /> {String(count).padStart(2, "0")}</span>
        </div>
      </div>
    </section>
  );
